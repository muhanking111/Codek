...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	async getDefaultLanguageModel(extension: IExtensionDescription, forceResolveModels?: boolean): Promise<vscode.LanguageModelChat | undefined> {
		...
		// [Codek] First pass: prefer the Copilot default (original behavior).
		// Second pass: accept any vendor's default-for-chat model so non-Copilot providers (e.g. codek)
		// can serve as the default language model when Copilot is disabled.
