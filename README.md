...
- **多 Agent 协作可控**：围绕任务计划、并发执行、权限审批和证据归档管理多 Agent 工作流，而不是让多个自动化过程在项目里失控运行。
...
多 Agent 场景下，Codek 的目标不是追求“全自动无人值守”，而是把多个 Agent 的任务拆解、队列状态、权限边界、命令输出和交付证据放到同一个审计链里。当前公开版本保留了相关编排源码和验证入口，但仍按公开预览边界处理，适合审阅设计与本机试用，不应视为已经完成长期稳定承诺的企业级调度系统。
